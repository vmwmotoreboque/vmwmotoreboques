...
async function handleRequest(request) {
    ...
    if (request.method === 'GET') {
        try {
            ...
            if (config) return new Response(JSON.stringify(config), { headers });
            const defaultConfig = {
                ate20: 120,
                km20a40: 2,
                base40: 150,
            ...
            return new Response(JSON.stringify({ error: 'Erro ao ler' }), { status: 500, headers });
        }
    }
    ...
    if (request.method === 'POST') {
        try {
            ...
            return new Response(JSON.stringify({ error: 'Erro ao salvar' }), { status: 500, headers });
        }
    }
